...
class Trainer:
    def __init__(self, options):
        ...
        model_101, input_size = self.initialize_model(model_name, num_classes, feature_extract, use_pretrained=True)
        ...
        noise_img = np.random.normal(loc=0, scale=1, size=(3, 512, 512))
        noise_img = np.expand_dims(noise_img, axis=0)
        noise_img = torch.from_numpy(noise_img).float()
        noise_img = noise_img.to('cuda')

        target_img = np.random.normal(loc=0, scale=1, size=(3, 512, 512))
        ...
        '''
        model = model_ft
        model.layer1.register_forward_hook(get_activation('fc2'))
        x = torch.randn(1, 25)
        x = noise_img
        output = model(x)
        ...
        for i in range(I):
            w = 10 * (I - i) / I
            L_ANN = self.compute_AAN_loss(x_o, x_t)
  #          g =
            sys.exit()
    ...
    def compute_AAN_loss(self, inputs, targets):
        print("compute AAN loss")
        M_o = []
        M_s = []
        G_o = []
        G_t = []
        ...
        output = self.models["resnet50"](inputs)
        ...
        self.models["resnet50"].layer3[5].conv3.register_forward_hook(get_activation('res4f')) # res4f
        self.models["resnet50"].layer4[2].conv3.register_forward_hook(get_activation('res5c')) # res5c
        output = self.models["resnet50"](targets)
    ...
    def generate_style_image(self, inputs):
        print("input original", inputs.shape)
        c = inputs.shape[1]
